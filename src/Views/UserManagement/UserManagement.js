...
export default function UserManagement() {
  ...
  const submitStatus = () => {
    let request = {
      id: userDetail.id,
      status: userDetail.status != 2 ? 2 : 1,
    };
    Post(changeStatus, token, request)
      .then((res) => {
        setLoading(false);

        navigate("/userManagement", {
          state: type == "general" ? "general" : "corporatefromAdd",
        });
      ...
      {statusModal && (
        <Modal tittle="Are you  sure ?" hide={() => setStatusModal(false)}>
          <div className="flex items-center justify-between">
            <p>
              You want to {userDetail.status === 1 ? "deactivate" : "activate"}{" "}
              this user`s account ?{" "}
            </p>
          </div>
          <div className="flex items-center justify-between">
